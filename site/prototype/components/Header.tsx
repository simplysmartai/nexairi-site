    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex justify-between items-center transition-all duration-300 ${isScrolled ? 'h-20' : 'h-28'}`}>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-baseline gap-2 leading-none">
                <span className="font-sans font-bold text-3xl md:text-4xl tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-gray-100 to-gray-400 drop-shadow-sm">
                  Nexairi
                </span>
                <span className="font-sans text-3xl md:text-4xl tracking-widest text-brand-cyan font-light uppercase">
                  Mentis
                </span>
              </div>
            </div>
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex space-x-8" role="navigation" aria-label="Desktop Navigation">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.label}
                href={item.href}
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <nav className="md:hidden bg-brand-dark border-b border-brand-border shadow-xl absolute w-full backdrop-blur-xl" aria-label="Mobile Navigation">
