...
export const Home: React.FC = () => {
  ...
  const featured = filteredPosts.filter(p => p.isFeatured).slice(0, 3);
  // Grid posts are everything excluding the top featured ones (to avoid duplication)
  const gridSource = filteredPosts.filter(p => !featured.includes(p));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Featured Section (Only show on 'All' or if featured posts exist in category) */}
      ...
      <div className="flex items-end justify-between border-b border-white/10 pb-4 mb-8">
        <div>
           <h2 className="text-3xl font-bold text-white mb-1">
             {activeCategory === 'All' ? 'Latest Dispatches' : `${activeCategory} Feed`}
           </h2>
      ...
      <InfiniteScroll
        ...
        className="!overflow-visible" // Override infinite scroll hidden overflow which cuts off shadows
      >
      ...
      {gridSource.length === 0 && (
        <div className="py-20 text-center border border-dashed border-white/10 rounded-xl">
          <p className="text-gray-500">No content signals detected for this frequency.</p>
        </div>
      )}
